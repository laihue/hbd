(function(window){
    ...
    Heart = function() {
        ...
        for (var i = 10; i < 30; i += 0.2) {
            ...
            //x=sin(4*t)+ cos(t),y=sin(t)+ cos(4*t),
            x=Math.sin(4*t)+Math.cos(t);
            y=Math.cos(4*t)+Math.sin(t);
    ...
    Heart.prototype = {
        get: function(i, scale) {
            return this.points[i].mul(scale || 8);
        }
    }
    ...
    Seed.prototype = {
        ...
        drawHeart: function() {
            ...
            for (var i = 0; i < heart.figure.length; i++) {
                var p = heart.figure.get(i, scale*6);
                ctx.lineTo(p.x, -p.y);
            }
            ctx.closePath();
            ctx.fill();
        ...
        drawText: function() {
            ...
            ctx.fillText("Come Brother", 23, 10);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
